...
const DetailsJob = () => {
  ...
  const handleRemoveJob = (jobId: JobId) => () => {
    removeJob(jobId)
  }
      ...
      {loading ? (
        ...
        <section className='w-full h-screen flex flex-col items-center font-light text-sm p-4'>
          ...
          <header className='w-full h-16 flex items-center justify-between'>
            ...
            <Box>
              <button onClick={job?.isFavorite ? handleRemoveJob(job?.id) : handleSaveJob(job)}><IconBookMar fill={job?.isFavorite ? '#ffc300' : '#9CA3AF'} /></button>
            </Box>
          </header>
