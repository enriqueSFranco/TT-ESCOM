...
class UserToken(APIView):
    def get(self, request, *args, **kwargs):
        username = request.GET.get('t100_name')
        try:
            user_token = Token.objects.get(user=StudentTokenSerializer(
            ).Meta.model.objects.filter(username=username).first())
...
class Login(ObtainAuthToken):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if login_serializer.is_valid():
            ...
            Response({'error': 'Nombre de usuario o password incorrectos'},
